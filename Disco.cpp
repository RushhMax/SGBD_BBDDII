...
void Disco::crearBloques() {
    ...
    for (int i = 0; i < nroBloques; i++) {
        ...
        for (int j = 0; j < nroSectoresBLOQUE; j++) {
            ...
            if (nPlato % nroPlatos == 1 && nPlato != 1) {
                nPlato = 1;
                nSector++;
            }
            if (nSector % this->nroSectores == 1 && nSector != 1) {
                nSector = 1;
                nPista++;
            }
            if (nPista % this->nroPistas == 1 && nPista != 1) {
                nPista = 1;
            }
        }
...
void Disco::adicRelacion(std::string _archivo) {
    ...
    int opc;
    std::cout << " > MANUAL(0) o AUTOMATICO(1)> ";
    std::cin >> opc;
    if (opc == 0) {
        int longMax;
        ...
        while (std::getline(str, linea, delimi)) {
            _diccionario << "#" << linea;
            std::cout << " TIPO DE DATO " << linea << " ? ";
            std::cin >> linea;
            _diccionario << "#" << linea;
            std::cout << " LONG MAX DE DATO " << linea << " ? ";
            std::cin >> linea;
            _diccionario << "#" << linea;
        }
    } else {
        ...
        while (std::getline(str, dato, delimi)) {  // obtengo ENCABEZADO
            ...
            if (isalpha(tip_dato[0])) {
                _diccionario << "#string#30";
            }  // si dato comienza con char -> string
            else if (is_IntFlo(tip_dato) == 'T') {
                _diccionario << "#int#4";
            }  // si dato es entero
            else if (is_IntFlo(tip_dato) == 'F') {
                _diccionario << "#float#4";
            }  // si dato es float
            else if (is_IntFlo(tip_dato) == 'S') {
                _diccionario << "#string#30";
            }  // si dato es string
        }
    }
    ...
    do {
        ...
        switch (opcion) {
            ...
            case 2:
                ...
                break;
            case 3:
                std::cout << " Que bloque quiere consultar? > ";
                std::cin >> n;
                consultarBloque(n);
                break;
            case 4:
